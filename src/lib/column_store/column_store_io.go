...
func SaveIntsToColumns(tb *TableBlock, dirname string, same_ints map[int16]ValueMap) {
	...
	for k, v := range same_ints {
		...
		col_fname := fmt.Sprintf("%s/%s.int.gb", dirname, GetBlockStringForKey(tb, k))
...
func SaveSetsToColumns(tb *TableBlock, dirname string, same_sets map[int16]ValueMap) {
	for k, v := range same_sets {
		...
		col_fname := fmt.Sprintf("%s/%s.set.gb", dirname, GetBlockStringForKey(tb, k))
...
func SaveStrsToColumns(tb *TableBlock, dirname string, same_strs map[int16]ValueMap) {
	for k, v := range same_strs {
		...
		col_fname := fmt.Sprintf("%s/%s.str.gb", dirname, GetBlockStringForKey(tb, k))
...
func SaveInfoToColumns(tb *TableBlock, dirname string) {
...
func SeparateRecordsIntoColumns(tb *TableBlock) SeparatedColumns {
	records := tb.RecordList
...
func SaveToColumns(tb *TableBlock, filename string) bool {
	dirname := filename
	...
	separated_columns := SeparateRecordsIntoColumns(tb)
	end := time.Now()
	Debug("COLLATING BLOCKS TOOK", end.Sub(start))

	SaveIntsToColumns(tb, partialname, separated_columns.ints)
	SaveStrsToColumns(tb, partialname, separated_columns.strs)
	SaveSetsToColumns(tb, partialname, separated_columns.sets)
	SaveInfoToColumns(tb, partialname)
	...
	start = time.Now()
	nb := LoadBlockFromDir(tb.Table, partialname, nil, false)
	end = time.Now()
	...
	if DEBUG_RECORD_CONSISTENCY {
		nb = LoadBlockFromDir(tb.Table, partialname, nil, true)
...
func unpackStrCol(tb *TableBlock, dec FileDecoder, info SavedColumnInfo) {
	...
	col.ValStringIDLookup = string_lookup
	...
	if into.BucketEncoded {
		...
		for _, bucket := range into.Bins {
			...
			for _, r = range bucket.Records {
				...
				if DEBUG_RECORD_CONSISTENCY {
					if record.Populated[col_id] != NO_VAL {
	...
	if into.BucketEncoded {
		for _, bucket := range into.Bins {
			...
			for _, r := range bucket.Records {
				...
				if DEBUG_RECORD_CONSISTENCY {
					if records[r].Populated[col_id] != NO_VAL {
