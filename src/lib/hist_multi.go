...
func (h *MultiHist) TrackPercentiles() {
	...
	// logarithmically
	numHists := 0
	for t := BucketSize; t > int64(NumBuckets); t >>= HistFactorPow {
		numHists += 1
	}
	...
	h.Subhists[numHists] = h.table.NewBasicHist(&info)
