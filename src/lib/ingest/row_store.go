...
func LoadSavedRecordsFromLog(t *Table, filename string) []*SavedRecord {
	common.Debug("LOADING RECORDS FROM LOG", filename)
	...
	if err != nil {
		common.Debug("ERROR LOADING INGESTION LOG", err)
	}
...
func AppendRecordsToLog(t *Table, records RecordList, blockname string) {
	...
	common.Debug("SAVING RECORDS", len(marshalled_records), "TO INGESTION LOG")
	...
	common.Debug("SERIALIZED INTO LOG", filename, network.Len(), "BYTES", "( PER RECORD", network.Len()/len(marshalled_records), ")")
	...
	for i := 0; i < 3; i++ {
		...
		err = common.RenameAndMod(w.Name(), fullname)
		if err == nil {
	...
	common.Warn("COULDNT INGEST INTO ROW STORE")
}
